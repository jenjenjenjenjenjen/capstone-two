...
const BASE_URL = 'https://api.nasa.gov/mars-photos/api/v1/rovers/perseverance';
...
router.get('/photos', async function(req, res) {
    ...
    req.query.earth_date ? baseQuery = baseQuery + `&earth_date=${req.query.earth_date}` : null;
    req.query.camera ? baseQuery = baseQuery + `&camera=${req.query.camera}` : null;
